"""
Lab 2b: Logistic Riemann Zero Extraction — Dual Config Version

Reads Vivado simulation output logistic_trajectory.txt (or logistic_trajectory_pure.txt)
Auto-searches and displays TWO configurations:
  Config A: Most precise (lowest MAE, typically 3 zeros)
  Config B: Most zeros (8 zeros, slightly higher MAE)

Usage: python analyze_logistic_zeros.py [trajectory_file]
"""
import numpy as np
...
# 1. Load
# ============================================================
fname = sys.argv[1] if len(sys.argv) > 1 else 'logistic_trajectory.txt'
if not os.path.exists(fname) and os.path.exists('logistic_trajectory_pure.txt'):
    fname = 'logistic_trajectory_pure.txt'
...
# ============================================================
# 2. Markov — auto search (dual config)
# ============================================================
print("\n" + "="*50)
...
all_results = []
bins_list = [300, 500, 800, 1000]
eps_list = [0.005, 0.008, 0.01, 0.015, 0.02, 0.025, 0.03, 0.04]
...
for nb in bins_list:
    for ep in eps_list:
        ...
        try:
            ...
            if r and r['n'] >= 3:
                all_results.append((nb, ep, r))
                if r['mae'] < 2.0:
                    print(f"  [{cnt}/{total}] bins={nb}, eps={ep:.3f} -> "
                          f"{r['n']} zeros, MAE={r['mae']:.4f}")
        except:
            pass

def print_config(label, nb, ep, r):
    print(f"\n  === {label} ===")
    print(f"  bins={nb}, eps={ep}")
    print(f"  {r['n']} zeros, MAE = {r['mae']:.4f}")
    print(f"  {'n':<5} {'Predicted':<12} {'True':<12} {'Error':<10} {'Rel%':<8}")
    print(f"  {'-'*47}")
    for i in range(r['n']):
        rel = r['errors'][i] / TRUE_ZEROS[i] * 100
        print(f"  {i+1:<5} {r['pred'][i]:<12.4f} {TRUE_ZEROS[i]:<12.4f} "
              f"{r['errors'][i]:<10.4f} {rel:<8.2f}")

# Config A: lowest MAE
all_results.sort(key=lambda x: x[2]['mae'])
cfg_a = all_results[0] if all_results else None

# Config B: most zeros, then lowest MAE
all_results.sort(key=lambda x: (-x[2]['n'], x[2]['mae']))
cfg_b = all_results[0] if all_results else None

if cfg_a:
    print_config("Config A: Most Precise", cfg_a[0], cfg_a[1], cfg_a[2])
if cfg_b and (cfg_b != cfg_a):
    print_config("Config B: Most Zeros", cfg_b[0], cfg_b[1], cfg_b[2])
...
# ============================================================
# 4. Plot — 3 panels: Config A, Config B, DMD
# ============================================================
print("\n" + "="*50)
print("Generating plots...")
print("="*50)

has_b = cfg_b and (cfg_b != cfg_a)
n_plots = 3 if has_b else 2
...
if cfg_a:
    r = cfg_a[2]
    x_pos = np.arange(r['n'])
    ax.bar(x_pos-0.15, TRUE_ZEROS[:r['n']], 0.3, label='True', color='steelblue', alpha=0.8)
    ax.bar(x_pos+0.15, r['pred'], 0.3, label='Markov', color='darkorange', alpha=0.8)
    for i in range(r['n']):
        ax.text(i, max(r['pred'][i], TRUE_ZEROS[i])+0.5,
                f'{r["errors"][i]:.2f}', ha='center', fontsize=8, color='red')
    ax.set_xticks(x_pos)
    ax.set_xticklabels([f'$t_{{{i+1}}}$' for i in range(r['n'])])
    ax.legend(fontsize=9)
ax.set_ylabel('Value')
ax.set_title(f'Config A: {cfg_a[2]["n"]} zeros, MAE={cfg_a[2]["mae"]:.3f}\n'
             f'(bins={cfg_a[0]}, eps={cfg_a[1]})', fontweight='bold')
...
if has_b:
    ...
    x_pos = np.arange(r['n'])
    ax.bar(x_pos-0.15, TRUE_ZEROS[:r['n']], 0.3, label='True', color='steelblue', alpha=0.8)
    ax.bar(x_pos+0.15, r['pred'], 0.3, label='Markov', color='#E74C3C', alpha=0.8)
    for i in range(r['n']):
        ax.text(i, max(r['pred'][i], TRUE_ZEROS[i])+0.5,
                f'{r["errors"][i]:.2f}', ha='center', fontsize=7, color='red')
    ax.set_xticks(x_pos)
    ax.set_xticklabels([f'$t_{{{i+1}}}$' for i in range(r['n'])])
    ax.legend(fontsize=9)
    ax.set_ylabel('Value')
    ax.set_title(f'Config B: {r["n"]} zeros, MAE={r["mae"]:.3f}\n'
                 f'(bins={cfg_b[0]}, eps={cfg_b[1]})', fontweight='bold')
    ax.grid(True, alpha=0.3, axis='y')
...
print(f"{'='*50}")
if cfg_a:
    print(f"  Config A (precise):  {cfg_a[2]['n']} zeros, MAE={cfg_a[2]['mae']:.4f}  "
          f"bins={cfg_a[0]}, eps={cfg_a[1]}")
if has_b:
    print(f"  Config B (most):     {cfg_b[2]['n']} zeros, MAE={cfg_b[2]['mae']:.4f}  "
          f"bins={cfg_b[0]}, eps={cfg_b[1]}")
