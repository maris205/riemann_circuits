"""
Analyze Henon SPICE output: Markov + Hankel-DMD
Usage: python analyze_henon.py henon_output.txt
"""
import numpy as np
...
# === Load data ===
fname = sys.argv[1] if len(sys.argv) > 1 else 'henon_output.txt'
...
# ============================================================
print('\n--- Hankel-DMD ---')
seg = traj[-5000:]  # tail segment (near critical point)
nd, nm = 150, 30
N = len(seg); M = N-nd
...
if pred is not None and n>=2:
    ...
    ax.legend()
ax.set_title(f'Henon Markov ({n} zeros)', fontweight='bold')
...
if scaled is not None and nd2>=2:
    ...
    ax.legend()
ax.set_title(f'Henon DMD ({nd2} modes)', fontweight='bold')
ax.set_xlabel('Zero index'); ax.set_ylabel('Value'); ax.grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig('henon_zeros.png', dpi=200)
plt.show()
print('\nSaved henon_zeros.png')
